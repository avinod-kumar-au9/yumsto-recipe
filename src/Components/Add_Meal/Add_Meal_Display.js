...
const Details = (props) => {
  ...
  const renderHover = (imgUrl, passingId) => {
    if (passingId === imageId) {
      return (
        <img className="resultlargeImg" src={imgUrl} alt="recipeHoverImage" />
      );
    }
  };
  ...
  const renderdate = () => {
    if (mealsApi) {
      if (mealsApi.meals.length > 0) {
        return mealsApi.meals
          .filter((fil) => {
            return JSON.parse(sessionStorage.getItem("email")) === fil.usermail;
          })
          .map((meal, idx) => {
    ...
    <div>
      <div className="addMealRow">
        <div className="mealcol">
          ...
          <small style={{ color: "yellow" }}>{duplicateRecipe}</small>
