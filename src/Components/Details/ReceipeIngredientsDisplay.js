              ...
              return (
                <React.Fragment key={idx}>
                  <div className="instructionsCol">
                    <h3>
                      <span className="material-icons">check_circle </span> Step{" "}
                      {instruc.number}
                    </h3>
                    <p>{instruc.step}</p>
                  </div>
      ...
      if (nutritionApiData.good.length > 0) {
        return nutritionApiData.good.map((good, idx) => {
          return (
            <React.Fragment key={idx}>
              <p>
                <strong>{good.title}:</strong> {good.amount}
              </p>
            </React.Fragment>
          );
        });
      ...
      if (nutritionApiData.bad.length > 0) {
        return nutritionApiData.bad.map((bad, idx) => {
          return (
            <React.Fragment key={idx}>
              <p>
                <strong>{bad.title}:</strong> {bad.amount}
              </p>
            </React.Fragment>
          );
        });
  ...
  const renderTasteReport = () => {
    if (tasteWidgetApiData) {
      return (
        <React.Fragment>
          <p>
            <strong>Sweetness</strong>: {tasteWidgetApiData.sweetness}
          </p>
          <p>
            <strong>Saltiness</strong>: {tasteWidgetApiData.saltiness}
          </p>
          <p>
            <strong>Sourness</strong>: {tasteWidgetApiData.sourness}
          </p>
          <p>
            <strong>Bitterness</strong>: {tasteWidgetApiData.bitterness}
          </p>
          <p>
            <strong>Savoriness</strong>: {tasteWidgetApiData.savoriness}
          </p>
          <p>
            <strong>Fattiness</strong>: {tasteWidgetApiData.fattiness}
          </p>
          <p>
            <strong>Spiciness</strong>: {tasteWidgetApiData.spiciness}
          </p>
        </React.Fragment>
      );
    }
